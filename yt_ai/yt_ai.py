    ...
    def run(self):
        ...
        for id, id_df in self.config.get_data().groupby(level=0):
            ...
            for model_tts in self.ttsDict:
                ...
                self.ttsDict[model_tts].create_audio(id_df.iat[0, 0], interval=0)
            ...
            for model_ttv in self.ttvDict:
                ...
                self.ttvDict[model_ttv].create_video(prompts_list[0], consider_audio=True)
            ...
            if id == 1:
                ...
                video_path = f"{self.config.get_config()['outPath']}/final_videos/{id-1}_final_with_subtitles.mp4"
            # subtitle_path = self.config.get_config()['dataFile']
            output_path = f"{self.config.get_config()['outPath']}/final_videos/{id}_final_with_subtitles.mp4"
            self.config.apply_subtitles(video_path, id_df, output_path)
